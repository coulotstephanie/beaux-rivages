...
const checklistItem = z
  .object({ id: z.string().min(1).max(80), label: z.string().min(1).max(180), done: z.boolean() })
  .strict();
export const housekeepingActionSchema = z.discriminatedUnion("action", [
  z
    .object({
      ...
      offlineRevision: z.number().int().min(0),
    })
    .strict(),
  z
    .object({
      ...
      status: z.enum(["approved", "correction_required"]),
    })
    .strict(),
  z
    .object({
  ...
  z
    .object({
      action: z.literal("create_inventory"),
      propertyId: z.string().uuid(),
      room: z.string().trim().min(1).max(120),
      category: z.string().trim().min(1).max(100),
      name: z.string().trim().min(1).max(180),
      quantity: z.number().int().min(0).max(10000),
      unitValueCents: z.number().int().min(0).max(10000000),
      condition: z.enum(["new", "good", "worn", "damaged", "missing"]),
    })
    .strict(),
  z
    .object({
      action: z.literal("record_linen"),
      propertyId: z.string().uuid(),
      reservationId: z.string().uuid().nullable(),
      item: z.string().trim().min(2).max(120),
      quantity: z.number().int().min(1).max(1000),
      direction: z.enum(["prepared", "sent_to_laundry", "returned", "discarded"]),
  ...
  z
    .object({
      ...
      taskType: z.enum(["cleaning", "quality", "arrival", "linen", "experience", "stock"]),
      checklist: z.array(checklistItem).min(1).max(80),
    })
    .strict(),
  z
    .object({
      action: z.literal("plan_intervention"),
  ...
  z
    .object({
      action: z.literal("update_intervention"),
      interventionId: z.string().uuid(),
      status: z.enum(["planned", "assigned", "postponed", "in_progress", "completed", "cancelled"]),
    })
    .strict(),
]);
