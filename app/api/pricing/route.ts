...
export async function POST(request: NextRequest) {
  ...
  if (!body || typeof body !== "object")
    return noStoreJson({ error: "Corps de requête invalide." }, { status: 400 });
  const input = body as Record<string, unknown>;
  const propertySlug = String(input.propertySlug);
  if (
    !isPropertySlug(propertySlug) ||
    !isIsoDate(input.arrival) ||
    !isIsoDate(input.departure) ||
  ...
  if (
    ![adults, children, babies, pets].every(Number.isInteger) ||
    adults < 1 ||
    adults + children > 8 ||
    babies < 0 ||
    pets < 0
  ...
  const options = Array.isArray(input.options)
    ? input.options.filter(
        (id): id is StayOptionId =>
          typeof id === "string" && validOptionIds.has(id as StayOptionId),
      )
    : [];
  const experiences = Array.isArray(input.experiences)
    ? input.experiences.filter(
        (id): id is BookingExperienceId =>
          typeof id === "string" && validExperienceIds.has(id as BookingExperienceId),
      )
    : [];
  return noStoreJson(
    await calculateQuote({
      ...
      experiences,
      promotionCode:
        typeof input.promotionCode === "string" ? input.promotionCode.slice(0, 40) : undefined,
    }),
  );
}
