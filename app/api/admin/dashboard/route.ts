...
export async function GET(request: NextRequest) {
  ...
  const houses = await Promise.all(
    propertySlugs.map(async (propertySlug) => {
      const [calendar, plan] = await Promise.all([
        synchronizePropertyCalendars(propertySlug, false, true),
        ratePlanRepository.get(propertySlug),
      ]);
      ...
      for (const block of calendar.blocks) {
        ...
        for (const day of enumerate(block.startsAt.slice(0, 10), block.endsAt.slice(0, 10))) {
          if (day >= startsOn && day < endsOn && !occupied.has(day))
            occupied.set(day, { provider, rate: rateForDate(plan, day).rate });
        }
      }
      const byProvider = [...occupied.values()].reduce<Record<string, number>>(
        (totals, value) => ({ ...totals, [value.provider]: (totals[value.provider] ?? 0) + 1 }),
        {},
      );
      const monthly = Array.from({ length: 12 }, (_, month) => {
        ...
        return {
          month: month + 1,
          occupiedNights: values.length,
          estimatedRevenue: values.reduce((sum, value) => sum + value.rate, 0),
        };
      });
      return {
        propertySlug,
        ...
        sourceNights: {
          airbnb: byProvider.airbnb ?? 0,
          booking: byProvider.booking ?? 0,
          abritel: byProvider.abritel ?? 0,
          direct: 0,
        },
        monthly,
  ...
  return noStoreJson({
    ...
    caveat:
      "Le revenu des plateformes reste estimé à partir des nuits bloquées : les flux iCal ne contiennent aucun montant. Le revenu direct confirmé provient de Supabase.",
  });
}
