...
export async function GET(request: NextRequest) {
  ...
  if (!isDatabaseConfigured()) return noStoreJson({ error: "Base non configurée." }, { status: 503 });
  try {
...
export async function POST(request: NextRequest) {
  ...
  try {
    return noStoreJson({ ok: true, result: await new HousekeepingRepository().execute(parsed.data) }, { status: 201 });
  } catch (error) {
