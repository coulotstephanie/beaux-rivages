...
export function StayPortal({ initialToken = "" }: { initialToken?: string }) {
  ...
  const pay = async (purpose: "deposit" | "balance" | "full-payment") => {
    ...
    try {
      ...
      if (!response.ok || !payload.checkoutUrl) return setMessage(payload.error ?? "Paiement indisponible.");
  ...
  return (
    <div className="stay-portal">
      ...
      <div className="stay-portal__grid">
        ...
        <section><h3>Vos expériences</h3>{stay.options.length ? <ul>{stay.options.map((option) => <li key={option}>{option}</li>)}</ul> : <p>Aucune attention ajoutée.</p>}</section>
