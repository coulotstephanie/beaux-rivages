...
const options = [
  ...
  ["basket", "Panier gourmand", 48, "selon saison", "option"],
...
export function StayComposer() {
  ...
  const total = useMemo(() => options.filter(([id]) => selected.includes(id)).reduce((sum, item) => sum + item[2], 0), [selected]);
  const bookingHref = useMemo(() => {
    const params = new URLSearchParams();
    const optionIds = options.filter(([id, , , , kind]) => kind === "option" && selected.includes(id)).map(([id]) => id);
    const experienceIds = options.filter(([id, , , , kind]) => kind === "experience" && selected.includes(id)).map(([id]) => id);
  ...
  return (
    <div className="stay-composer">
      <div className="stay-composer__options">
        {options.map(([id, label, price, note]) => <button type="button" key={id} onClick={() => toggle(id)} className={selected.includes(id) ? "is-selected" : ""} aria-pressed={selected.includes(id)}>
          <span>{selected.includes(id) ? "✓" : "+"}</span><strong>{label}</strong><small>à partir de {price} € · {note}</small>
        </button>)}
      </div>
