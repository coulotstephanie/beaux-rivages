...
export function guestComposition(data: GuestMessageData) {
  ...
  return parts.length === 2 ? parts.join(" et ") : parts.map((part, index) => index === parts.length - 1 ? `et ${part}` : part).join(", ").replace(", et ", " et ");
}

const experienceLabels = { essentiel: "Essentiel", confort: "Confort", signature: "Signature Beaux Rivages" };
const experienceCopy = {
  essentiel: "Votre séjour comprend tout l’essentiel pour profiter pleinement de la maison et découvrir les îles en toute liberté.",
  confort: "Vous avez choisi la formule Confort. Les prestations et les options sélectionnées seront préparées avec soin afin de faciliter votre arrivée et de rendre votre séjour encore plus agréable.",
  signature: "Vous avez choisi l’expérience Signature Beaux Rivages. Nous préparerons votre arrivée avec une attention toute particulière afin que votre séjour commence dès les premiers instants dans une atmosphère chaleureuse et personnalisée.",
};

export function optionLabels(data: GuestMessageData) {
  const o = data.selectedOptions;
  return [
    o.linenPackage && `Forfait linge pour ${o.linenGuestsCount ?? data.adults + data.children} personne${(o.linenGuestsCount ?? data.adults + data.children) > 1 ? "s" : ""}`,
    o.beachTowels && "Serviettes de plage", o.bathrobes && "Peignoirs", o.slippers && "Chaussons",
    o.earlyCheckIn && "Arrivée anticipée", o.lateCheckOut && "Départ tardif", o.personalizedArrival && "Arrivée personnalisée",
    o.aperitifBasket && "Panier Apéritif", o.sweetBasket && "Panier Douceur",
    o.romancePack && "Pack Romance", o.signatureRomancePack && "Pack Romance Signature", o.petOption && "Option animal",
  ].filter((item): item is string => Boolean(item));
}

const conditionalCopy = (data: GuestMessageData) => {
  const blocks: string[] = [];
  if (data.children || data.babies) blocks.push(`Nous avons bien noté que vous voyagerez avec ${[data.children && `${data.children} enfant${data.children > 1 ? "s" : ""}`, data.babies && `${data.babies} bébé${data.babies > 1 ? "s" : ""}`].filter(Boolean).join(" et ")}. Les équipements réservés ou inclus seront préparés avant votre arrivée.`);
  if (data.pets) blocks.push(`Nous sommes heureux d’accueillir également votre compagnon à quatre pattes.\n\nDes gamelles seront mises à votre disposition. Merci de prévoir son couchage personnel et de veiller à ce qu’il ne monte pas sur les lits.${data.propertyId === "nid-d-ete" ? "\n\nDans les parties communes de La Maison Heureuse, les chiens doivent impérativement être tenus en laisse." : ""}`);
  if (data.selectedOptions.linenPackage) blocks.push(`Le forfait linge a bien été enregistré pour ${data.selectedOptions.linenGuestsCount ?? data.adults + data.children} personne(s). Les lits et le linge prévus dans votre formule seront préparés selon les modalités de votre réservation.`);
  if (data.selectedOptions.personalizedArrival) blocks.push("Vous avez choisi l’Arrivée Personnalisée Beaux Rivages. Stéphanie ou Bruno prendra contact avec vous afin de confirmer l’horaire et les modalités de votre accueil.");
  if (data.selectedOptions.earlyCheckIn) blocks.push("Votre demande d’arrivée anticipée a bien été enregistrée. L’horaire définitif vous sera confirmé en fonction de l’organisation et de la disponibilité de la maison.");
  if (data.selectedOptions.romancePack || data.selectedOptions.signatureRomancePack) blocks.push("Une attention particulière sera préparée dans la maison conformément à votre réservation.");
  return blocks;
};
...
const cardText = (data: GuestMessageData) => {
  ...
  return [`Maison : ${p.name}`, `Arrivée : ${textDate(data.arrivalDate, data.locale)}`, `Départ : ${textDate(data.departureDate, data.locale)}`, `Durée : ${nights(data)} nuit${nights(data) > 1 ? "s" : ""}`, `Voyageurs : ${guestComposition(data)}`, `Formule : ${experienceLabels[data.experienceLevel]}`, ...(options.length ? [`Options : ${options.join(", ")}`] : [])].join("\n");
};
...
const stayCardHtml = (data: GuestMessageData) => `<div style="margin:26px 0;padding:22px;background:#f4f0e7;border-left:3px solid #b89a60"><strong style="font-size:12px;letter-spacing:.12em">VOTRE SÉJOUR</strong>${paragraphs(cardText(data))}</div>`;

export function renderGuestMessage(data: GuestMessageData, type: MessageType, secrets?: ArrivalSecrets, scheduledDate = type === "departure" ? new Date(Date.parse(`${data.departureDate}T12:00:00Z`) - 86_400_000).toISOString().slice(0, 10) : data.arrivalDate): GuestMessage {
  const p = properties[data.propertyId];
  const options = optionLabels(data);
  ...
  if (type === "booking_confirmation") {
    ...
    return { type, subject, preheader: `Votre séjour à ${p.name} est confirmé`, text, html: htmlShell(data, subject, `Votre séjour à ${p.name}`, body), idempotencyKey };
  }
  if (type === "departure") {
    const subject = "À bientôt… et merci pour votre séjour 🌊";
    const island = data.propertyId === "nid-d-ete" ? "l’Île d’Oléron" : "l’Île de Ré";
    const propertyClosing = data.propertyId === "chai-des-tortues"
    ...
    const htmlCards = cards.map((card) => `<div style="margin:18px 0;padding:22px;border:1px solid #e1dbce;border-radius:2px;background:#fff"><h2 style="margin:0 0 14px;font:400 23px Georgia,serif">${e(card.title)}</h2>${paragraphs(card.copy)}</div>`).join("");
    ...
    return { type, subject, preheader: `Quelques repères avant votre départ de ${p.name}`, text, html: htmlShell(data, subject, `Votre départ de ${p.name}`, body), idempotencyKey };
  }
