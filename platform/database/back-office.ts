import "server-only";
import type { BackOfficeReservation, BackOfficeSnapshot, ReservationStatus } from "@/platform/admin/contracts";
...
function nights(arrival: string, departure: string) {
  return Math.max(0, Math.round((Date.parse(`${departure}T12:00:00Z`) - Date.parse(`${arrival}T12:00:00Z`)) / 86_400_000));
}
...
export class SupabaseBackOfficeRepository {
  ...
  async snapshot(): Promise<BackOfficeSnapshot> {
    ...
    const reservations = ((reservationsResult.data ?? []) as Row[]).map((row): BackOfficeReservation => {
      ...
      return {
        id: String(row.id), reference: String(row.reference), propertyId: String(row.property_id),
        propertyName: String(property?.name ?? "Logement"), propertySlug: String(property?.slug ?? ""),
        status: row.status as ReservationStatus, channel: String(row.channel),
    ...
    for (const payment of paymentRows.filter((row) => ["paid", "authorized"].includes(String(row.status)))) {
    ...
    const contractByReservation = new Map(contractRows.map((row) => [String(row.reservation_id), row]));
    const active = reservations.filter((reservation) => !["cancelled", "declined"].includes(reservation.status));
    const confirmed = active.filter((reservation) => ["confirmed", "completed"].includes(reservation.status));
    ...
    const guestSummaries = guestRows.map((guest) => {
      const guestReservations = links.filter((link) => String(link.guest_id) === String(guest.id)).map((link) => reservationById.get(String(link.reservation_id))).filter((item): item is BackOfficeReservation => Boolean(item));
      return {
        id: String(guest.id), name: `${guest.first_name} ${guest.last_name}`, email: String(guest.email), phone: String(guest.phone ?? ""),
        stays: guestReservations.length, nights: guestReservations.reduce((sum, reservation) => sum + nights(reservation.arrival, reservation.departure), 0),
        pets: guestReservations.reduce((sum, reservation) => sum + reservation.pets, 0),
        lastStay: guestReservations.map((reservation) => reservation.departure).sort().at(-1) ?? null,
      };
    }).filter((guest) => guest.stays > 0);
    const blockRows = (blocksResult.data ?? []) as Row[];
    const propertyStats = propertyRows.map((property) => {
      const propertyReservations = confirmedThisYear.filter((reservation) => reservation.propertyId === String(property.id));
      const directDays = new Set(propertyReservations.flatMap((reservation) => enumerateDays(reservation.arrival, reservation.departure, yearStart, yearEnd)));
      const platformDays = new Set(blockRows.filter((block) => String(block.property_id) === String(property.id) && String(block.source) !== "reservation")
        .flatMap((block) => {
          const match = String(block.stay_range).match(/\[(\d{4}-\d{2}-\d{2}),(\d{4}-\d{2}-\d{2})\)/);
      ...
      return {
        id: String(property.id), slug: String(property.slug), name: String(property.name), status: String(property.status),
        occupancyRate: Math.round((occupiedNights / 365) * 10_000) / 100, occupiedNights, directNights, platformNights,
        revenueCents: propertyReservations.reduce((sum, reservation) => sum + reservation.totalCents, 0),
      };
    });
    const sourceRows = (sourcesResult.data ?? []) as Row[];
    const sourceById = new Map(sourceRows.map((row) => [String(row.id), row]));
    const recentErrors = [
      ...((syncsResult.data ?? []) as Row[]).filter((row) => Number(row.error_count) > 0 || row.status === "failed").map((row) => ({
        id: `sync-${row.id}`, area: "Calendrier", message: `${row.error_count} erreur(s) pendant la synchronisation`, occurredAt: String(row.started_at),
      })),
      ...((emailsResult.data ?? []) as Row[]).filter((row) => ["failed", "bounced"].includes(String(row.status))).map((row, index) => ({
        id: `email-${index}-${row.updated_at}`, area: "E-mail", message: String(row.last_error ?? `Statut ${row.status}`), occurredAt: String(row.updated_at),
      })),
      ...paymentRows.filter((row) => ["failed", "cancelled"].includes(String(row.status))).map((row) => ({
        id: `payment-${row.reservation_id}-${row.created_at}`, area: "Paiement", message: `Paiement ${row.status}`, occurredAt: String(row.created_at),
      })),
    ].sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).slice(0, 20);
    return {
      generatedAt: new Date().toISOString(), today,
      metrics: {
        revenueTodayCents: revenueFor(today), revenueMonthCents: revenueFor(month), revenueYearCents: revenueFor(String(year)),
        pendingPaymentsCents: active.reduce((sum, reservation) => sum + Math.max(0, reservation.totalCents - (paidByReservation.get(reservation.id) ?? 0)), 0),
        averageStayNights: confirmedThisYear.length ? Math.round(confirmedThisYear.reduce((sum, reservation) => sum + nights(reservation.arrival, reservation.departure), 0) / confirmedThisYear.length * 10) / 10 : 0,
        directShare: confirmedThisYear.length ? Math.round(confirmedThisYear.filter((reservation) => reservation.channel === "direct").length / confirmedThisYear.length * 10_000) / 100 : 0,
      },
      operational: {
        ...
        inHouse: active.filter((reservation) => reservation.arrival <= today && reservation.departure > today),
      ...
      pilotage: {
        ...
        recentSyncs: ((syncsResult.data ?? []) as Row[]).map((row) => {
          ...
          return { id: String(row.id), provider: String(source?.provider ?? "inconnu"), property: String(propertyById.get(String(source?.property_id))?.name ?? "Logement"), status: String(row.status), importedCount: Number(row.imported_count), errorCount: Number(row.error_count), startedAt: String(row.started_at) };
        }),
  ...
  async execute(input: AdminOperationInput) {
    if (input.action === "create_reservation") {
      ...
      const quote = {
        adults: input.adults, children: input.children, babies: input.babies, pets: input.pets,
        nightsTotalCents: input.totalCents, optionsTotalCents: 0, cleaningFeeCents: 0,
        touristTaxCents: 0, discountCents: 0, totalCents: input.totalCents,
        depositDueCents: deposit, balanceDueCents: input.totalCents - deposit,
        source: "back_office", propertySlug: input.propertySlug,
      };
      const { data: reservation, error } = await this.client.rpc("create_direct_reservation", {
        property_slug: input.propertySlug, arrival_date: input.arrival, departure_date: input.departure,
        guest: input.guest, quote, selected_options: [], request_key: crypto.randomUUID(),
      });
      if (error) {
        if (error.code === "23P01") throw new Error("DATES_UNAVAILABLE");
        throw new Error(`RESERVATION_WRITE_FAILED:${error.code}`);
      }
      const { data: updated, error: updateError } = await this.client.from("reservations").update({
        status: input.status, channel: input.channel, expires_at: null,
        confirmed_at: input.status === "confirmed" ? new Date().toISOString() : null,
      }).eq("id", reservation.id).select("id,reference").single();
    ...
    if (input.action === "block_dates") {
      ...
      const { data, error } = await this.client.from("occupancy_blocks").insert({
        property_id: property.id, stay_range: `[${input.arrival},${input.departure})`, source: "manual", note: input.note,
      }).select("id").single();
    ...
    const { data, error } = await this.client.from("reservations").update(patch).eq("id", input.reservationId).select("id,reference").single();
    if (error) {
      if (error.code === "23P01") throw new Error("DATES_UNAVAILABLE");
  ...
  private async property(slug: string) {
    const { data, error } = await this.client.from("properties").select("id").eq("slug", slug).single();
