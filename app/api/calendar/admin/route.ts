...
export async function GET(request: NextRequest) {
  ...
  const calendars = await Promise.all(
    propertySlugs.map(async (propertySlug) => {
      const sync = await synchronizePropertyCalendars(propertySlug, false, true);
      return { propertySlug, sources: sync.results };
    }),
  );
  return noStoreJson({
    configuration: getCalendarConfigurationStatus(),
    calendars,
    persistentEditingEnabled: false,
  });
}
...
export async function POST(request: NextRequest) {
  ...
  const propertySlug =
    body && typeof body === "object"
      ? String((body as Record<string, unknown>).propertySlug ?? "")
      : "";
  if (!isPropertySlug(propertySlug))
    return noStoreJson({ error: "Logement inconnu." }, { status: 400 });
  const result = await synchronizePropertyCalendars(propertySlug, true, true);
...
export async function PUT(request: NextRequest) {
  ...
  return noStoreJson(
    {
      error:
        "La modification persistante nécessite le dépôt chiffré CalendarSourceRepository. Les secrets de production restent administrés dans Vercel pour cette version.",
