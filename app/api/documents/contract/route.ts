...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!stay.documents.some((document) => document.id === documentId))
      return new Response("Document indisponible", { status: 404 });
    if (format === "html")
      return new Response(createContractHtml(stay), {
        headers: {
    ...
    const familyName = (stay.contractDetails?.travelerLastName ?? stay.travelerName)
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/[^a-z0-9]+/gi, "-")
      .toUpperCase();
    ...
    return new Response(Buffer.from(bytes), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="Reservation-BEAUX-RIVAGES-${year}-${familyName}.pdf"`,
        "Cache-Control": "private, no-store",
      },
    });
  } catch {
