...
export async function POST(request: NextRequest) {
  ...
  if (!stripeMode) {
    return noStoreJson({ error: "Stripe n’est pas entièrement configuré." }, { status: 503 });
  }
  const parsed = refundSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success)
    return noStoreJson({ error: "Demande de remboursement invalide." }, { status: 400 });
  try {
    ...
    if (amountCents > remaining)
      return noStoreJson(
        { error: "Le remboursement dépasse le montant restant." },
        { status: 400 },
      );
    const refund = await new StripePaymentAdapter().refund({
      paymentProviderId: payment.provider_payment_id!,
      amountCents,
    ...
    await new SupabaseAuditRepository().record({
      ...
      entityType: "payment",
      entityId: payment.id,
      metadata: { amountCents, refundId: refund.refundId, environment: stripeMode },
    });
    return noStoreJson({
      ok: true,
    ...
    return noStoreJson({ error: "Le remboursement n’a pas pu être créé." }, { status: 502 });
  }
}
