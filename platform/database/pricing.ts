...
export class SupabasePricingPlanReader {
  async get(propertySlug: PropertySlug): Promise<PropertyRatePlan> {
    ...
    const propertyResult = await client.from("properties")
      .select("id,slug,currency").eq("slug", propertySlug).single();
    if (propertyResult.error) throw new Error(`PRICING_PROPERTY_FAILED:${propertyResult.error.code}`);
    const property = propertyResult.data;
    const [ratesResult, optionsResult, promotionsResult, overridesResult] = await Promise.all([
      client.from("rates").select("*,seasons(*)").eq("property_id", property.id).eq("enabled", true),
      client.from("property_options").select("price_cents,enabled,options(code)").eq("property_id", property.id),
      client.from("promotions").select("*").eq("property_id", property.id),
      client.from("yield_rate_overrides").select("stay_date,nightly_rate_cents,minimum_nights").eq("property_id", property.id).eq("status", "active"),
    ]);
    ...
    if (promotionsResult.error) throw new Error(`PRICING_PROMOTIONS_FAILED:${promotionsResult.error.code}`);
    if (overridesResult.error) throw new Error(`PRICING_OVERRIDES_FAILED:${overridesResult.error.code}`);
    ...
    const seasons = rates.flatMap((rate) => {
    ...
    const promotions = promotionsResult.data.map((promotion): Promotion => {
      ...
      if (promotion.kind === "long_stay") return { ...common, kind: "long-stay", minimumNights: promotion.minimum_nights ?? 1 };
      if (promotion.kind === "last_minute") return { ...common, kind: "last-minute", maximumLeadDays: promotion.maximum_lead_days ?? 0 };
      if (promotion.kind === "early_booking") return { ...common, kind: "early-booking", minimumLeadDays: promotion.minimum_lead_days ?? 0 };
      if (promotion.kind === "code") return { ...common, kind: "code", code: promotion.code ?? "" };
      return {
        ...common,
        kind: "seasonal",
        ranges: parseDateRange(typeof promotion.valid_range === "string" ? promotion.valid_range : null),
      };
    });

    const optionPrices = Object.fromEntries(optionsResult.data.flatMap((item) =>
    ...
    return {
      ...
      touristTax: {
        ...
        mode: baseRate.tourist_tax_mode === "percentage" ? "percentage" : "fixed-per-adult-per-night",
        value: Number(baseRate.tourist_tax_value),
      },
      optionPrices,
      seasons,
      promotions,
      overrides: overridesResult.data.map((item) => ({ date: item.stay_date, nightlyRate: item.nightly_rate_cents / 100, minimumNights: item.minimum_nights ?? undefined })),
    };
  }
}
