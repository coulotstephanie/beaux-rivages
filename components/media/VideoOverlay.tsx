...
export function VideoOverlay({ locale = "fr" }: { locale?: SupportedLocale }) {
    ...
    >
      <motion.h1 variants={item} transition={transition}>
        <span className={`immersive-overlay__story ${messageVisible ? "is-visible" : ""}`}>
          ...
          <span className="immersive-overlay__story-accent">{tr(locale, message.accent)}</span>
        </span>
      </motion.h1>
      <motion.p variants={item} transition={transition}>
        {tr(locale, "Des maisons de caractère, une hospitalité inspirée de trois générations d’excellence entre l’Île de Ré et l’Île d’Oléron.")}
      </motion.p>
      <motion.div variants={item} transition={transition} className="immersive-overlay__actions">
        <Link href="#maisons" className="immersive-button immersive-button--primary">
          {tr(locale, "Découvrir nos maisons")}
        </Link>
        <Link href={localizedHref(locale, "/reserver")} className="immersive-button">
          {tr(locale, "Réserver")}
        </Link>
      </motion.div>
    </motion.div>
