...
export class StripePaymentAdapter implements RefundGateway {
  async createCheckout(input: { stay: StayAccessPayload; purpose: PaymentPurpose; amountCents: number; paymentId: string; idempotencyKey: string }) {
    ...
    return stripe.checkout.sessions.create({
      ...
      line_items: [{ quantity: 1, price_data: { currency: "eur", unit_amount: input.amountCents, product_data: { name: `${input.purpose} · ${input.stay.propertyName}`, description: `Réservation ${input.stay.reference}` } } }],
      metadata: { paymentId: input.paymentId, reservationId: input.stay.reservationId, reference: input.stay.reference, purpose: input.purpose },
      payment_intent_data: { metadata: { paymentId: input.paymentId, reservationId: input.stay.reservationId, reference: input.stay.reference, purpose: input.purpose } },
