...
test("vacation rental schema publishes the three explicitly approved precise locations", () => {
  for (const slug of ["chai-des-tortues", "villa-raie-manta", "nid-d-ete"]) {
    assert.match(seo, new RegExp(`"${slug}": \\{[\\s\\S]*?streetAddress`));
  }
  assert.match(seo, /streetAddress: propertyLocation\?\.streetAddress/);
  assert.match(seo, /postalCode: propertyLocation\?\.postalCode/);
  assert.match(seo, /latitude: propertyLocation\.latitude/);
  assert.match(seo, /longitude: propertyLocation\.longitude/);
  assert.match(seo, /latitude: 46\.157711/);
